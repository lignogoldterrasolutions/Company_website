...
export default function TechnologyContent({ nutrients }: Props) {
  ...
  const processDetail = [
    ...
    { emoji: '💰', accent: '#5C3D11', title: 'Step 3 — Dual Value Output', desc: 'Facilities immediately benefit from scale-free pipes and up to 80% water savings. Spent cores become nutrient-saturated assets.' },
    ...
    <>
      ...
      <section className="py-20 section-light-img">
        <div className="max-w-6xl mx-auto px-6">
          ...
          <motion.h2 
            ...
            className="text-3xl font-bold mb-4 font-playfair text-dark-brown">Nutrients Lost in Every Flush</motion.h2>
          <motion.p 
            initial={{ opacity: 0, y: 20 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true, margin: '-100px' }}
            transition={{ duration: 0.6, delay: 0.15 }}
            className="text-base leading-relaxed mb-10 max-w-2xl text-dark-brown/70 font-inter">
            ...
            {nutrients.map((n, i) => (
              ...
              >
                <GradientCard className="h-full text-center">
                  ...
                  <div className="text-2xl font-bold mb-1 font-playfair text-dark-brown">{n.range}</div>
                  <div className="text-base font-semibold mb-3 font-inter text-dark-brown">{n.name}</div>
                  <p className="text-sm leading-relaxed text-dark-brown/60 font-inter">{nutrientRoles[i]}</p>
                </GradientCard>
              </motion.div>
            ))}
          </div>
      ...
      <section className="py-20 bg-white">
        <div className="max-w-6xl mx-auto px-6">
          ...
          <h2 className="text-3xl font-bold mb-10 font-playfair text-dark-brown">How the Ligno-Pod Works</h2>
            ...
            {features.map((f, i) => (
              ...
              >
                <GradientCard className="h-full">
                  ...
                  <h3 className="text-sm font-semibold mb-2 font-inter text-dark-brown">{f.title}</h3>
                  <p className="text-xs leading-relaxed text-dark-brown/70 font-inter">{f.desc}</p>
                </GradientCard>
              </motion.div>
            ))}
          </div>
      ...
      <section className="py-20 section-light-img">
        <div className="max-w-6xl mx-auto px-6">
          ...
          <h2 className="text-3xl font-bold mb-8 font-playfair text-dark-brown">The Four-Step Process — Detailed</h2>
          <div className="flex flex-col gap-4">
            {processDetail.map((s, i) => (
              <motion.div 
                ...
                transition={{ duration: 0.6 }}
              >
                <GradientCard className="flex gap-5 items-start">
                  ...
                  <div>
                    ...
                    <p className="text-sm leading-relaxed text-dark-brown/70 font-inter">{s.desc}</p>
                  </div>
                </GradientCard>
              </motion.div>
            ))}
      ...
      <section className="py-20 bg-white">
        <div className="max-w-6xl mx-auto px-6">
          ...
          <h2 className="text-3xl font-bold mb-8 font-playfair text-dark-brown">Solar Thermal Drying System</h2>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
            <motion.div
              ...
              transition={{ duration: 0.6 }}
            >
              <p className="text-base leading-relaxed mb-5 text-dark-brown/70 font-inter">
              ...
              <ul className="flex flex-col gap-2.5 mb-6 list-none">
                {['Compact and cost-effective', 'Requires only sunlight', 'Easy to operate and maintain', 'Ideal for small-scale users'].map((b, i) => (
                  <li key={i} className="flex items-center gap-3 text-sm text-dark-brown/70 font-inter">
                    <span className="text-earth-green font-bold text-lg">✓</span>{b}
                  </li>
                ))}
              </ul>
              <div className="rounded-xl p-4 text-sm leading-relaxed mb-3 bg-ligno-gold/10 border border-ligno-gold/25 text-dark-brown/75 font-inter">
                <strong className="text-dark-brown font-semibold">Heat at 60°C</strong> effectively kills bacteria, viruses, and other pathogens.
              </div>
              <div className="rounded-xl p-4 text-sm leading-relaxed bg-earth-green/7 border border-earth-green/20 text-dark-brown/75 font-inter">
            ...
            >
              <GradientCard className="p-6">
                <h3 className="text-base font-semibold mb-4 font-inter text-dark-brown">Setup Details</h3>
                {solarSpecs.map(([k, v], i) => (
                  <div key={i} className="flex justify-between py-2.5 text-sm font-inter" style={{ borderBottom: i < solarSpecs.length - 1 ? '1px solid rgba(44,26,6,0.1)' : 'none' }}>
                    <span className="text-dark-brown/55">{k}</span>
                    <span className="font-semibold text-dark-brown">{v}</span>
                  </div>
                ))}
              </GradientCard>
            </motion.div>
      ...
      <section className="py-20 section-light-img">
        <div className="max-w-6xl mx-auto px-6">
          ...
          <h2 className="text-3xl font-bold mb-3 font-playfair text-dark-brown">In-Situ Nutrient Mineralization</h2>
          <p className="text-base mb-10 max-w-xl text-dark-brown/65 font-inter">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {evidence.map((e, i) => (
              <motion.div 
                ...
                transition={{ duration: 0.6, delay: i * 0.15 }}
              >
                <GradientCard className="p-0 overflow-hidden hover:shadow-xl transition-shadow duration-300">
                  <div className="relative h-48 w-full bg-cream overflow-hidden border-b border-dark-brown/8">
                    <Image
                      src={e.image}
                      alt={e.title}
                      fill
                  ...
                  <div className="p-5">
                    <h4 className="text-sm font-semibold mb-2 font-inter text-dark-brown">{e.title}</h4>
                    <p className="text-xs leading-relaxed text-dark-brown/65 font-inter">{e.desc}</p>
                  </div>
                </GradientCard>
              </motion.div>
            ))}
