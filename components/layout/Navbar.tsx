...
export default function Navbar() {
  ...
  return (
    <motion.nav
      ...
      style={{ background: '#FAF7F0', borderBottom: '1px solid rgba(44,26,6,0.1)' }}
    >
      <div className="max-w-6xl mx-auto px-6 flex items-center justify-between h-16">
          ...
          {NAV_LINKS.map(l => (
            <li key={l.href}>
              <Link
                ...
                style={{ color: pathname === l.href ? '#C89527' : '#2C1A06' }}
              >
                {l.label}
                <span
        ...
        <div className="hidden md:block">
          <Link href="/contact">
            <button
              className="text-sm font-semibold px-5 py-2 rounded-lg text-white transition-all duration-200 bg-ligno-gold hover:bg-dark-brown"
        ...
        {/* Mobile hamburger */}
        <button className="md:hidden p-2" onClick={() => setOpen(!open)} style={{ color: '#2C1A06' }}>
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            style={{ background: '#FAF7F0', borderColor: 'rgba(44,26,6,0.1)' }}
          >
            <div className="px-6 py-4 flex flex-col gap-1">
              {NAV_LINKS.map(l => (
                <Link
                  ...
                  style={{
                    color: pathname === l.href ? '#C89527' : '#2C1A06',
                    borderColor: 'rgba(44,26,6,0.07)',
                  }}
                >
                  {l.label}
                </Link>
              ...
              <div className="pt-3">
                <Link href="/contact" onClick={() => setOpen(false)}>
                  <button
                    className="w-full text-sm font-semibold px-5 py-2.5 rounded-lg text-white bg-ligno-gold hover:bg-dark-brown transition-all duration-200"
                    style={{ fontFamily: 'Inter, sans-serif' }}
                  >
                    Contact Us
                  </button>
